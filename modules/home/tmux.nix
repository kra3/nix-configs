{
  flake.homeManagerModules.home-tmux =
    ...
    lib.mkMerge [
      ...
      # session ("0") so resurrect restores over it instead of colliding with a restored
        ...
        launchd.agents.tmux-server = {
          ...
          config = {
            ProgramArguments = [
              "${pkgs.bash}/bin/bash"
              "-c"
              "${pkgs.tmux}/bin/tmux ls >/dev/null 2>&1 || ${pkgs.tmux}/bin/tmux new-session -d"
            ];
            RunAtLoad = true;
          };
        };
      ...
      (lib.mkIf pkgs.stdenv.isLinux {
        systemd.user.services.tmux-server = {
          ...
          # oneshot + RemainAfterExit, no ExecStop: a kill-server on unit restart (hm
          # switch, tmux store-path bump) would discard every unsaved session.
          Service = {
            Type = "oneshot";
            RemainAfterExit = true;
            ExecStart = "${pkgs.bash}/bin/bash -c '${pkgs.tmux}/bin/tmux ls >/dev/null 2>&1 || ${pkgs.tmux}/bin/tmux new-session -d'";
          };
        };
      })
    ];
