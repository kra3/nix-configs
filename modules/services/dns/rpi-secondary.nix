    ...
    in
    {
      ...
      services.adguardhome = {
        ...
        settings = {
          ...
          # Steven Black's registry copy: redundant with Dan Pollock's/AdGuard DNS filter already in
          # this set) -- combined they're ~70% of sutala's full list's volume and OOM this board's 1GB
          # RAM. Everything else (ads/trackers/phishing/malware/scam/stalkerware) stays intact.
          filters = lib.filter (
            f:
            !(builtins.elem f.id [
              1767444741
