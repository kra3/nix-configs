...
build_popup() {
    ...
    local A=(--remove '/network\.pop\..*/'
        ...
        --set network.pop.hdr icon="󰖩" icon.color=0xff89b4fa icon.font="MesloLGS Nerd Font:Bold:13.0"
            label="$ssid" label.color=0xfff9e2af label.font="Helvetica Neue:Bold:13.0" label.align=left
    ...
    for kv in "IP|$ip" "Subnet|$mask" "Router|$router"; do
        ...
        A+=(--add item "network.pop.$k" popup."$NAME"
            --set "network.pop.$k"
                icon="$k" icon.font="Helvetica Neue:Regular:12.0" icon.color=0xffa6adc8
                icon.align=left icon.width="$half" icon.padding_left=12
                label="$v" label.font="Menlo:Regular:12.0" label.color=0xffcdd6f4
                ...
                click_script="printf '%s' \"\$(sketchybar --query network.pop.$k | jq -r .label.value)\" | pbcopy; sketchybar --set network.pop.$k label.color=0xffa6e3a1; (sleep 0.8; sketchybar --set network.pop.$k label.color=0xffcdd6f4) &")
    done
    sketchybar "${A[@]}"
}
