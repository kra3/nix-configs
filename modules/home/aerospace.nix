  ...
  flake.homeManagerModules.home-aerospace = { ... }: {
    programs.aerospace = {
      ...
      settings = {
        ...
        # Notify SketchyBar's workspace indicators on every switch.
        ...
        gaps = {
          ...
          outer.left = 8;
          outer.bottom = 8;
          # Static gap (height 36 + margin 8 + y_offset 4 from the SketchyBar
          # config) instead of Omachy's per-monitor notch-detection daemon,
          # which mutates aerospace.toml on disk -- incompatible with a
          # Nix-store-managed config file.
          outer.top = 48;
          outer.right = 8;
        };
