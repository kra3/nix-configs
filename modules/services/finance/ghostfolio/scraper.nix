{
  flake.nixosModules.services-finance-ghostfolio-scraper =
    {
      config,
      lib,
      pkgs,
    ...
    let
      scraperPython = pkgs.python3.withPackages (ps: [
        ps.requests
        ps.psycopg2
      ]);
    ...
    in
    {
      ...
      environment.etc."alloy/ghostfolio-scraper.alloy".text =
        flakeLib.observability.mkAlloyJournalSource
          {
