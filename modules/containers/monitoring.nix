{
  flake.nixosModules.containers-monitoring =
    {
      config,
      lib,
      pkgs,
    ...
    }:
    {
      ...
      services.nginx.virtualHosts."grafana.${config.vars.acme.domain}" =
        lib.mkIf (config.containers.monitoring.config.services.grafana.enable or false)
          (
      ...
      services.nginx.virtualHosts."loki.${config.vars.acme.domain}" =
        lib.mkIf (config.containers.monitoring.config.services.loki.enable or false)
          (
            flakeLib.nginx.mkProxyVhost {
              domain = config.vars.acme.domain;
      ...
      sops.secrets."monitoring.grafana.oidc_client_secret" =
        lib.mkIf (config.containers.monitoring.config.services.grafana.enable or false)
          {
            mode = "0440";
            group = "jellyfin";
      ...
      systemd.services."container@monitoring" = flakeLib.container-definition.mkContainerSystemdDeps [ ];
    };
}
